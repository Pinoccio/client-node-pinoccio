...
module.exports = function(args,api,config,cb){
  ...
  function fork(port){
    ...
    connected = proc = cp.spawn(node,[file,bridge,'--worker','--port',port],opts);
    ...
    proc.stderr.on('end',function(){
      ...
      if(l.indexOf("Error: Cannot open") > -1){
        cannotOpen = true;
      } else if(l.indexOf('"code":"SERIAL_TIMEOUT"')){
  ...
  function conBridge(port){
    ...
    var cb = function(err){
      if(err) {
        ...
        if(error.indexOf("bitlash: did not find prompt after") > -1 ){
          code = "SERIAL_TIMEOUT";
        }
