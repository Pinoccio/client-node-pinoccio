...
module.exports = function(args,api,config,cb){
  ...
  if(!args.worker){
    ...
    var pinoccios = [];
    var watchs = serialWatcher(serialport);
    ...
    process.on('message',function(o){
      ...
      b.bridgeCommand(o.command,function(err,data){
        ...
        if(err) console.log("<child> bridge command from parent ",o.command,'got error ',err);
        else console.log("<child> bridge command successful",o.command);
  ...
  function fork(port){
    ...
    connected.on('message',function(o){
      ...
      console.log('<MASTER MESSAGE >',o);

      if(o.type == "ping"){
        console.log("SETTING PING VERIFY TO NOW");
    ...
    var ping;
    var pingsRunning = false;
    var startPings = function(){

      if(pingsRunning){
        return console.log("TRYING TO START PINGS WHEN PINGS IS ALREADY STARTED")
      }
  ...
  function conBridge(port){
    ...
    var s = bridge(serialport,port,opts,function(err,b){
    ...
    s.on('data',function(data){
      ...
      try{
        if(data && data.type == "report" && data.report.type == "uptime"){
          console.log('<child> yay ping',data);
