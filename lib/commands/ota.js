...
module.exports = function(args,api,config,cb){
  ...
  if (args.target) {
    ...
    return cb('Must specify either --target or --broadcast');
  }
    ...
    if(device) {
      ...
      o.connect(device,function(err,bl){
        ...
        if (args.reboot)
          ...
          // TODO: Delay to let the bootloader boot (retries currently
          // make this work, but adding 1s delay here would be nicer).
        ...
        function send_end() {
          ota_command('ota.end', null, function() { console.log("Done"); process.exit(); });
        }
