...
module.exports = function(config,reconnect){
  ...
  function getConnection(cb){
    console.log('recon: get connection!')
    if(reconnect && !reconnect._bound) {

      console.log('recon: reconnect now bound')
      ...
      reconnect.on('connect',function(s){
        process.nextTick(function(){
          ...
          console.log('recon: bound connection listener!');
          o.connection.mdm.on('connection',function(stream){
            console.log('recon: mdm got stream from connection listener',stream.meta);
          ...
          console.log('recon: calling all pending callbacks for a connection ',pending);
          while(pending.length) pending.shift()(false,o.connection);
        });
      }).on('disconnect',function(){
        console.log('recon: disconnected');
        o.connection = false;
    ...
    if(o.connection) return process.nextTick(function(){
      console.log('recon: returning connection')
      if(o.connection) cb(false,o.connection);
      else pending.push(cb);
    });

    console.log('recon: queuing callback for connection');
    pending.push(cb);
  }
