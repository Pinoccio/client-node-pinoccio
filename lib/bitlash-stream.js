...
module.exports = function(serialPort){
  ...
  serialPort.on('data',function(buf){
    ...
    if(bindexof(buf,thprefix) == 0)
    {
      buf = buf.slice(thprefix.length);
      var packet = telehash.pdecode(new Buffer(buf.toString(),"hex"));
      if(packet)
      {
        if(packet.js.type == "ping")
        {
          var js = {type:"pong",from:telehash.parts};
          var out = telehash.pencode(js,telehash.keys["1a"]);
          console.log("writing pong",out.toString("hex"));
          serialPort.write("telehash(\""+out.toString("hex")+"\")"+"\n");
        }
      }
    }
    if(state == 'prompt') {
