...
module.exports = function(serialPort){
  ...
  telehash.deliver("local",function(to,msg){
    console.log("writing bytes",msg.length);
    var out;
    while(msg.length)
    {
      out = "telehash(\""+msg.slice(0,100).toString("hex")+"\")\n";
      console.log(out);
...
//    console.log("BUF",buf);
    ...
    {
      var raw = new Buffer(buf.slice(bindexof(buf,thprefix)+thprefix.length).toString(),"hex");
      ...
      if(packet)
      {
        ...
        if(packet.js.type == "ping")
        {
          ...
          serialPort.write(out+"telehash(0)\n");
        }else{
