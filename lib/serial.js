  ...
  var o = {
    ...
    connect:function(com,cb){
      serialConnect(com,function(err,port){
        ...
        // if serial has been idle for 30 secs and i have no prompt.
        (function fn(){
          if(opened) return;
          var t = Date.now();          
          if(t-bl.lastData >= 30000) {
            cb(new Error('bitlash: did not find prompt after serial was idle for over 30 seconds.'));
            cb = function(){};
          } else {
